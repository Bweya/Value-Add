...
def trade():
    ...
    workbook = xlsxwriter.Workbook(str(getmonth).upper()+str(year)+"_ValueAdd_Report.xlsx")
    ...
    for check in hqFXDealNumbers:
        for i in USDIndex_get_DealNumbers:
            ...
            if check == FX133[48][i] and FXhqrades[7][j] != 'DKK':
                ...
                if periodUS < 0 and (Currency[j][:3] == 'MMK' or Currency[j][:3] == 'ETB' or Currency[j][:3] == 'GNF' or Currency[j][:3] == 'JMD' or Currency[j][:3] == 'MGA'):
                    ...
                    for check133_dealno in FX133[48]:
                        ...
                        if (indexed > USDstart_index and indexed < USDstop_index) or (indexed > EURstart_index and indexed < EURstop_index):
                            ...
                            if check == check133_dealno:

                                worksheet.write('M'+str(row_record), "{:,.2f}".format(FX133[35][indexed]), the_columns)

                                worksheet.write('N'+str(row_record), "{:,.2f}".format(FX133[20][indexed]), the_columns)
                                worksheet.write('O'+str(row_record), "{:,.2f}".format(FX133[13][indexed]), the_columns)
                                worksheet.write('P'+str(row_record), "{:,.2f}".format(FXhqrades[11][j]), the_columns)
                                worksheet.write('Q'+str(row_record), "{:,.2f}".format((FX133[20][indexed])-(FX133[13][indexed])), the_columns)
                                try:
                                    sum += (FX133[20][indexed])-(FX133[13][indexed])
                                except e:
                                    print("NaN values")
                        indexed += 1
        ...
        for i in EURIndex_get_DealNumbers:
            ...
            if check == FX133[48][i] and FXhqrades[7][j] != 'DKK':
                ...
                if periodEU < 0 and (Currency[j][:3] == 'MMK' or Currency[j][:3] == 'ETB' or Currency[j][:3] == 'GNF' or Currency[j][:3] == 'JMD' or Currency[j][:3] == 'MGA'):
